...
function styles(el){try{var s=getComputedStyle(el);return{display:s.display,position:s.position,color:s.color,background:s.backgroundColor,fontFamily:s.fontFamily,fontSize:s.fontSize,fontWeight:s.fontWeight,borderRadius:s.borderRadius,opacity:s.opacity};}catch(_){return{};}}
