...
export const SHARINGAN_CAPTURE_RESOURCE_BUNDLE_PROTOCOL = "dezin.sharingan-capture-resource-bundle.v3" as const;
...
function scope(value: unknown): SharinganCaptureBundleScope {
  const item = exact(value, [
    "projectId", "captureId", "inputHash", "workspaceId", "resourceId",
    "nodeId", "title", "resourceKind",
  ], "Sharingan Capture bundle scope");
  if (typeof item.inputHash !== "string" || !SHA256.test(item.inputHash)
    || item.resourceKind !== "sharingan-capture") fail("Sharingan Capture bundle scope is invalid");
  return Object.freeze({
    projectId: identifier(item.projectId, "Sharingan Capture Project id"),
    captureId: identifier(item.captureId, "Sharingan Capture id"),
