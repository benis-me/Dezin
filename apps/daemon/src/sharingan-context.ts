...
export function buildSharinganSystemPrompt(): string {
  return [
    ...
    "1. The bounded measured SOURCE + region-plan JSON printed by `node .sharingan/probe.mjs source-scaffold --stdout`.",
    "2. `node .sharingan/probe.mjs source-summary`.",
    "3. Generated `src/sharingan-regions/*` files when Dezin has already split the page into measured source regions.",
    "4. Visual QA source-vs-result findings.",
    "5. Optional `outline` or `render-map` output only for one targeted ambiguity or repair.",
    "",
    "Hard rules:",
    "- `.sharingan` and `public/_assets` are immutable read-only Revision sidecars. Never write, delete, rename, refresh, navigate, capture, click, or scroll through the live probe API in this Task.",
    "- The only allowed probe commands are offline reads: `source-summary`, `source-scaffold --stdout`, `outline`, and `render-map`.",
    "- The stdout scaffold JSON is measured source material; it is not the final artifact and must never be written back into a pinned root.",
    "- Use its `source.boxes`, `source.images`, `source.vectors`, `source.texts`, and `regionPlan` to understand measured sizes, colors, order, and regions.",
    ...
    "2. Run `node .sharingan/probe.mjs source-scaffold --stdout` and consume the complete bounded JSON directly; it performs no writes.",
    "3. Use the stdout scaffold and any existing region files only as geometry/style/source reference.",
