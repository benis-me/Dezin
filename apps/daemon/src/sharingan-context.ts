...
export function buildSharinganSystemPrompt(): string {
  return [
    ...
    "- Components are allowed when they directly map to captured source regions. Do not create a generic marketing layout, design-system demo, or guessed product UI.",
    ...
    "3. Read the reference scaffold files only as geometry/style/source reference.",
...
export function buildSharinganContext(input: SharinganContextInput): SharinganContext {
  ...
  const promptBlock = [
    ...
    "2. `node .sharingan/probe.mjs source-scaffold` (this writes the measured reference scaffold; it is not optional on a fresh project)",
    ...
    "",
    "The generated reference scaffold contains `const SOURCE =` with `SOURCE.boxes`, `SOURCE.images`, `SOURCE.vectors`, and `SOURCE.texts`. Use it to generate normal Standard React source in `src/`. Do not leave the scaffold replay as the final app, and do not replace measured source regions with guessed semantic UI.",
    "",
