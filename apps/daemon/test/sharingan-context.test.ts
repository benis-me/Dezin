...
test("buildSharinganContext points the agent at the dezin-probe CLI (not hand-written curl), with budget", () => {
  ...
  assert.match(promptBlock, /probe\.mjs source-scaffold/); // measured scaffold before freehand coding
...
test("the prompt steers away from drowning in dom.json — outline is the blueprint, build fast", () => {
  ...
  assert.match(promptBlock, /preserve the generated `SOURCE` scaffold, run the build, and stop/i); // push to build, not analyze
});
...
test("buildSharinganContext caps source analysis before writing source-derived components", () => {
  ...
  assert.match(promptBlock, /must start from the generated scaffold/i);
  assert.match(promptBlock, /Source Component Inventory/i);
  assert.match(promptBlock, /measured scaffold/i);
  assert.match(promptBlock, /Do not create `src\/components\/` during the first build pass/i);
  ...
  assert.match(promptBlock, /preserve or extract from the `SOURCE` scaffold data/i);
...
test("buildSharinganSystemPrompt bypasses normal Standard design generation behavior", () => {
  ...
  assert.match(prompt, /SOURCE object is canonical/i);
  assert.match(prompt, /Do not replace the scaffold/i);
  assert.match(prompt, /Do not create `src\/components\/` during the first build pass/i);
  assert.match(prompt, /Ignore any generic Standard\/design-system\/craft instruction/i);
  assert.match(prompt, /Do not rewrite it into a semantic component library/i);
