...
export interface DesignFlowNodeData extends Record<string, unknown> {
  ...
  /** Hover-highlight elements in the focused preview and report clicks as annotation targets. */
...
export function DesignCanvasNode({ data, selected }: NodeProps<DesignFlowNode>) {
  ...
  useEmbeddedPreviewContextMenuChannel({
    ...
    annotateMode: focusSource && annotateMode,
    onLayout: (message) => {
      if (!material) onContentAspectRatio?.(node.id, message.width / message.height);
    },
    onEscape: () => {
      if (focusSource) onPreviewEscape?.(node.id);
    },
