...
项目状态、画布历史、生成 Version、素材、Agent 对话、Job 和 Export 都保存在本地磁盘。Dezin 不提供托管账户、遥测、模型路由或付费推理服务。你选择的第三方编程 Agent CLI 仍可能按照其自身认证和政策访问对应模型服务。
...
- **Fail-closed 验证。** Node HTML 和 Export source 必须依次通过路径 allowlist、URL/DOM capability、CSP、严格 TypeScript、隔离 Vite build、构建产物扫描及 Chrome 桌面/移动视觉门禁后才能发布。Export 验证刻意聚焦安全与可复现性，不等同于旧 `@dezin/quality` taste linter。
- **有界恢复。** Export 最多续跑一次未完成或超时的构建，并最多执行一次带精确诊断的 repair。身份漂移、冻结输入变化、未授权路径及重复验证失败都会直接终止。

早期的 `skills × design systems × craft` 生成流水线、分阶段 Research direction gate、确定性 lint→repair、Prototype/Standard 模式、分支 variants 与 ZIP 交付仍以历史或独立模块形式保留在仓库中，但不再代表当前 Design Canvas runtime。当前架构以 [`docs/DESIGN-CANVAS.md`](./docs/DESIGN-CANVAS.md) 为准；[`docs/DESIGN-PROCESS.md`](./docs/DESIGN-PROCESS.md) 是已归档的前代方案。
...
packages/
  ...
  quality/   独立/旧 artifact taste 与 geometry 检查
  research/  独立/旧分阶段 Research 工具
  prompt/    旧分层 prompt 组合
  skills/    旧 SKILL.md loader
  craft/     旧 anti-slop craft 文档生成器
```
