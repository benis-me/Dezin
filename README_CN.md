...
- **两种构建模式。** *Prototype（原型）* —— 单个自包含的 HTML 文件，迭代最快。*Standard（标准）* —— 一个真正的 Vite + React 项目（预装 `motion` + `gsap`），带组件和路由。
...
前置要求：**Node ≥ 22.16**、**pnpm 11**，以及至少一个位于 **PATH 上、且已登录的编码 Agent CLI**（例如 `claude`），用于真实生成。
...
Node 套件使用 `node --experimental-strip-types --experimental-sqlite --test`，Web 使用带 V8 覆盖率的 Vitest。根编排器显式列出每个套件，设置有界超时，并在失败时终止自己拥有的进程组。CI 固定使用 Node 22.16 / pnpm 11.9，并对生产依赖执行 high 级别审计。
