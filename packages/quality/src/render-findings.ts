 ...
 * P0 findings are listed first.
 */
export function renderFindingsForAgent(findings: Finding[]): string | null {
  ...
  lines.push(
    "Re-write the artifact file to fix every P0 (and ideally P1), then stop. " +
      "Do not explain — just emit the corrected file.",
  );
  lines.push("</artifact-lint>");
  return lines.join("\n");
}
