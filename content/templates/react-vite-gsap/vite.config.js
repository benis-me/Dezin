...
function onScroll(){refreshSelected();reportScroll();}
function init(){mkbox();document.addEventListener('mousemove',move,true);document.addEventListener('click',click,true);document.addEventListener('scroll',onScroll,true);window.addEventListener('scroll',reportScroll,{passive:true});window.addEventListener('resize',refreshSelected);document.addEventListener('keydown',function(e){if((on||pinned||selectedEl)&&e.key==='Escape'){parent.postMessage({source:'dezin',type:'cancel'},'*');mode(false);clearMark();}},true);window.addEventListener('message',function(e){var d=e.data;if(!d||d.source!=='dezin-parent')return;if(d.type==='select-mode')mode(!!d.on);else if(d.type==='clear')clearMark();else if(d.type==='focus-target')focusTarget(d.selector,d.rect);else if(d.type==='sync-scroll')setScroll(d.top,d.left);});}
